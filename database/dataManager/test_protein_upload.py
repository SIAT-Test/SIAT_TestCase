...
class test_protein_upload(unittest.TestCase):
    pre = test_pre()
    pre.login()

    # 修改文件标识字段，保证每次上传文件不重复
    def file_rename_acc(self):
        with open('D:/TestCase/database/dataManager/protein_backup.gp', 'r', encoding='utf-8') as f_old:
            with open('D:/TestCase/database/dataManager/protein_testRW.gp', 'w', encoding='utf-8') as f_new:
                for line in f_old:
                    if "testRW" in line:
                        i = line.index("testRW")
    ...
    def protein_upload(self):
        user_id = self.pre.get_userID()
        file = open('D:/TestCase/database/dataManager/protein_testRW.gp', 'rb')
        self.file_rename_acc()

        data = {"token": "BPNF", "user_id": user_id, "database_type_name": "protein",
                "source_name": "NCBI.PROTEIN", "create_type": "import",
                'file': ("1.gp", file)}
    ...
    def recover_remove(self, bioEntryId):
        ...
        resp_rmv_id = requests.delete(url=homepage + "/api/nucleicAcid/remove", headers=headers,
                                      data=json.dumps({"ids": [bioEntryId], "sourceIds": [19]})).json()
    ...
    def test_debug(self):
        acc = self.protein_upload()
        bioId = self.protein_audit(acc)
