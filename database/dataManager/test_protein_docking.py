...
class test_protein_docking(unittest.TestCase):
    pre = test_pre()
    pre.login()

    # 调用接口/ardb/upload_id，外部对接导入核酸数据
    def protein_docking(self, data_id: list):
        ...
        for d in data_id:
            resp_whether_exist = requests.get(url=homepage + "/api/nucleicAcid/page?", headers=headers,
                                              params={"pageNum": 1, "pageSize": 10, "sort": "Default_order",
                                                      "sourceDatabase": "protein", "_t": time_stamp,
                                                      "queryStrList": json.dumps([
                                                          {"field_name": "Accession", "term": "", "value": d}])}).json()
            self.assertEqual(resp_whether_exist["code"], 200, resp_whether_exist)
        ...
        data = {"token": "BPNF", "user_id": user_id, "database_type_name": "protein",
                "source_name": "NCBI.PROTEIN", "create_type": "docking", "gis": data_id_str}
    ...
    def recover_remove(self, whether_exist: dict):
        ...
        for accession in whether_exist:
            if not whether_exist[accession]:
                ...
                self.assertEqual(resp_get_bioId["code"], 200, resp_get_bioId)
                ...
                resp_rmv_id = requests.delete(url=homepage + "/api/nucleicAcid/remove", headers=headers,
                                              data=json.dumps({"ids": [rmv_id], "sourceIds": [19]})).json()
    ...
    # 每次随机外部导入1个数据，循环20次
    def test_protein_dock1(self):
        i = 0
        while i < 20:
            i += 1
            rand_acc = random.randint(1, 88888)
            acc_dock = ["KKD" + str(rand_acc).zfill(5)]

            self.protein_docking(acc_dock)

    # 一次性导入max个(10)数据
    def test_protein_dock_max(self):
        acc_dock = []
        ...
        while i < 10:
            i += 1
            rand_acc = random.randint(1, 88888)
            acc_dock.append("KKD" + str(rand_acc).zfill(5))
